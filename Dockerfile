...
RUN wget 'https://repo.continuum.io/miniconda/Miniconda3-py37_4.8.3-Linux-x86_64.sh' -O miniconda.sh && \
    bash miniconda.sh -b -p /opt/miniconda3 && \
    rm -f miniconda.sh

# 3. 将Miniconda的可执行文件目录添加到系统PATH
...
RUN npm install -g configurable-http-proxy@4.2.0 bower@1.8.9 qs@6.9.5 less@3.12.0 && rm -rf ~/.npm

# 4.安装必要的依赖 
# --- 核心修改：先执行 setup_repo.sh 进行 git clone ---
COPY setup_repo.sh /root/setup_repo.sh
RUN chmod +x /root/setup_repo.sh && \
    /bin/bash /root/setup_repo.sh


# --- 然后执行 setup_env.sh 进行依赖安装 ---
...
RUN echo "source activate && conda activate base" >> /root/.bashrc
